...
/// Simpan jawaban kuesioner di memori (sementara).
...
final scoreRepositoryProvider =
    Provider<ScoreRepository>((ref) => ScoreRepository());

/// Use case untuk konversi skor → level risiko.
final calculateRiskProvider =
    Provider<CalculateRiskLevel>((ref) => CalculateRiskLevel());

/// Skor mentah dari questionnaire.
...
/// Hasil akhir — threshold seragam: 0–9 Rendah, 10–19 Sedang, 20–30 Tinggi.
final resultProvider = Provider<MentalResult>((ref) {
  ...
  if (score >= 20) {
    recommendation =
        'Kondisi Anda memerlukan perhatian serius. Segera konsultasikan dengan profesional kesehatan mental.';
  } else if (score >= 10) {
    recommendation =
        'Kelola stres dengan baik, perbaiki pola hidup, dan pertimbangkan konsultasi ke psikolog.';
  } else {
    recommendation =
        'Kondisi Anda tergolong baik. Pertahankan kebiasaan positif dan jaga kesehatan mental.';
  }
