...
class ScreeningPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final userAge = user?.age ?? 21;
    final questions = ref.watch(questionsProvider(userAge));
    final qState = ref.watch(questionnaireProvider);

    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (qState.activeQuestions.isEmpty) {
        ref.read(questionnaireProvider.notifier).loadQuestionsByAge(userAge);
      }
    });

    final progressPercent = questions.isEmpty
        ? 0.0
        : (qState.answers.length / questions.length).clamp(0.0, 1.0);
    final progressLabel =
  ...
  void _showResetDialog(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Semua progres screening saat ini akan dihapus.',
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('KOREKSI', style: TextStyle(color: Colors.white38)),
          ),
          FilledButton(
